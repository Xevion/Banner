 ...
 *
 * Only top-level files are re-exported: nested ones are foreign types that
 * exist to satisfy another binding's import, not part of this surface.
 */
function generateBarrel(root: string): { types: number; changed: boolean } {
  ...
  // Every per-type file carries ts-rs's own header; without one here the barrel
  // reads as hand-maintained and edits to it disappear on the next check.
  const barrel = [
    "// Generated from the ts-rs output. Do not edit this file manually.",
    "// Regenerate with: just bindings",
    "",
    ...types.map((t) => `export type { ${t} } from "./${t}";`),
    "",
  ].join("\n");

  const path = join(dir, "index.ts");
  const stale = !existsSync(path) || readFileSync(path, "utf8") !== barrel;
  if (stale) writeFileSync(path, barrel);
  return { types: types.length, changed: stale };
}

/**
 * Fail when two Rust types would claim the same binding filename.
 *
 * ts-rs names each file after the type, so two `TimeRange`s in different
 * modules overwrite each other and one vanishes from the barrel. Their
 * generated test names collide the same way, which is where it is cheapest
 * to notice.
 */
async function assertExportsAreUnique(ctx: RunContext): Promise<void> {
  ...
  for (const line of listed.stdout.split("\n")) {
    const match = /^(.+)::(export_bindings_\w+): test$/.exec(line.trim());
    if (!match) continue;
    const [, module, test] = match;
    owners.set(test, [...(owners.get(test) ?? []), module]);
  }

  const clashes = [...owners]
    .filter(([, modules]) => modules.length > 1)
    .map(([test, modules]) => {
      const name = test.replace("export_bindings_", "");
      return `  ${name}: ${modules.join(", ")}`;
    });

  if (clashes.length > 0) {
    ctx.fail(
      `Types sharing one binding file, rename all but one:\n${clashes.join("\n")}`,
    );
  }
}
...
async function regenerateBindings(ctx: RunContext): Promise<number> {
  ...
  await assertExportsAreUnique(ctx);
  ...
  try {
    ...
    const barrel = generateBarrel(ctx.cwd);
    if (barrel.changed) changed++;
    ctx.log(`${barrel.types} types, ${changed} changed`);
    return 0;
  } finally {
...
export default defineConfig({
  ...
  tasks: withTargetDirs(withSubsystemAliases([
    ...
    task({
      ...
      // The barrel is an input as well as an output: tempo's cache only checks
      // that an output exists, so editing one by hand would otherwise stick.
      inputs: [...RUST_SOURCES, `${BINDINGS_DIR}/index.ts`],
