...
- **Long operations**: Always `defer()` before doing async work, so users see a loading state instead of a timeout
- **Responses**: Ephemeral for user-specific data, public for shareable results
- **Error messages**: User-friendly, no stack traces, but log the full error server-side
