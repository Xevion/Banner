...
let dismissed = $state<DuplicatePair[]>(untrack(() => data.dismissed));
...
const pairKey = (pair: DuplicatePair) => `${pair.survivor.id}-${pair.loser.id}`;

async function refresh() {
  const res = await client.getInstructorDuplicates();
  if (res.isOk) {
    pairs = res.value.pairs;
    dismissed = res.value.dismissed;
    error = null;
  } else {
...
async function handleDismiss(pair: DuplicatePair) {
  actionLoading = pairKey(pair);
  const res = await client.dismissDuplicate(pair.survivor.id, pair.loser.id);
  actionLoading = null;

  if (res.isOk) {
    result = {
      message: `Recorded #${pair.survivor.id} and #${pair.loser.id} as different people.`,
      isError: false,
    };
    await refresh();
  } else {
...
async function handleUndismiss(pair: DuplicatePair) {
  actionLoading = pairKey(pair);
  const res = await client.undismissDuplicate(pair.survivor.id, pair.loser.id);
  actionLoading = null;

  if (res.isOk) {
    result = {
      message: `Returned #${pair.survivor.id} and #${pair.loser.id} to review.`,
      isError: false,
    };
    await refresh();
  } else {
  ...
  <div class="mb-4 flex items-start justify-between gap-4">
    <div>
      ...
      <p class="mt-1 text-sm text-muted-foreground">
        ...
        Merging moves every section, link and candidate onto the surviving record; marking a
        pair as different people keeps it off this list until the decision is undone.
      </p>
    </div>
    {#if safeCount > 0}
      <button
  ...
  {#if dismissed.length > 0}
    <details class="mt-6">
      <summary class="cursor-pointer text-sm font-medium text-muted-foreground">
        Marked as different people ({dismissed.length})
      </summary>
      <div class="mt-3 space-y-3">
        {#each dismissed as pair (pairKey(pair))}
          {@render card(pair, true)}
        {/each}
      </div>
    </details>
  {/if}
...
{#snippet card(pair: DuplicatePair, isDismissed: boolean)}
  {@const key = pairKey(pair)}
  {@const busy = actionLoading === key}
  <div class="rounded-md border border-border bg-card p-3 {isDismissed ? 'opacity-75' : ''}">
    <div class="mb-2 flex items-center justify-between gap-2">
      ...
      <div class="flex shrink-0 items-center gap-1">
        {#if isDismissed}
          <button
            onclick={() => void handleUndismiss(pair)}
        ...
        {:else}
          <button
            ...
            disabled={actionLoading !== null}
            class="inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted disabled:opacity-50 cursor-pointer"
          >
            {#if busy}
              <LoaderCircle size={12} class="animate-spin" />
            {:else}
    ...
    <p class="mb-2 text-xs text-muted-foreground">
      {isDismissed ? "Reviewed and kept apart as two people." : TIER_HINT[pair.tier]}
    </p>

    <div class="grid gap-2 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
      {@render side(pair.loser, isDismissed ? null : "absorbed")}
      {#if !isDismissed}
        <ArrowRight size={16} class="mx-auto hidden text-muted-foreground sm:block" />
      {:else}
        <span class="mx-auto hidden text-xs text-muted-foreground sm:block">&middot;</span>
      {/if}
      {@render side(pair.survivor, isDismissed ? null : "kept")}
    </div>
  </div>
{/snippet}

{#snippet side(person: DuplicateSide, role: string | null)}
  <div
    class="rounded border p-2 text-xs {role === 'kept'
