...
describe("QueryController", () => {
  ...
  describe("fetch", () => {
    ...
    it("updates error and preserves previous data on error", async () => {
      ...
      // Second fetch fails. Data is preserved.
      await controller.fetch();
      expect(controller.data).toEqual({ count: 42 });
  ...
  describe("stale response handling", () => {
    it("ignores stale responses when a new fetch starts", async () => {
      ...
      // Start second fetch, which should mark first as stale
      ...
      // First completes late, so it should be ignored
  ...
  describe("debounce", () => {
    ...
    it("does not set isLoading during debounce wait", async () => {
      ...
      await vi.advanceTimersByTimeAsync(300);
      // After debounce fires, fetch starts. Because it's async and resolved,
      // isLoading may have flipped, so just check fetcher was called.
      expect(fetcher).toHaveBeenCalledTimes(1);
    });
