    ...
    for key in stale_keys {
        ...
        match &key {
            ComputedCacheKey::Stats { period, term } => {
                recompute_stats(entry, pool, &key, period, term.as_deref(), update_tx).await;
            }
            ComputedCacheKey::Timeseries { period, bucket, term } => {
                recompute_timeseries(entry, pool, &key, period, bucket, term.as_deref(), update_tx).await;
            }
            ComputedCacheKey::Subjects => {
                recompute_subjects(entry, pool, &key, events, reference_cache, update_tx).await;
            }
        }
    }
}
...
async fn recompute_stats(
    ...
    match scraper_stats::compute_stats(pool, period, term).await {
        Ok(raw) => {
...
async fn recompute_timeseries(
    ...
    match scraper_stats::compute_timeseries(pool, period, Some(bucket), term).await {
