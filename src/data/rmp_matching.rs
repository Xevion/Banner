...
fn is_known_abbreviation(subject: &str) -> bool {
    matches_known_abbreviation(subject, "")
...
pub async fn generate_candidates(db_pool: &PgPool) -> Result<MatchingStats> {
    ...
    let subject_rows: Vec<(i32, String, i64)> = sqlx::query_as(
        r#"
    ...
    for (iid, subject, count) in subject_rows {
        subject_map
            .entry(iid)
            .or_default()
    ...
    // Sibling profiles held back until every claimant is known.
    let mut inherited: Vec<(i32, i32, f32)> = Vec::new();
    let mut claimants: HashMap<i32, HashSet<i32>> = HashMap::new();
        ...
        if distinct_people.len() == 1 {
            ...
            if let Some(name) = identified {
                for (legacy_id, ms) in &scored {
                    ...
                    if same_person {
                        inherited.push((*instructor_id, *legacy_id, ms.score));
                    }
                }
            }
    ...
    for (instructor_id, legacy_id, score) in inherited {
        let uncontested = claimants.get(&legacy_id).is_none_or(|ids| ids.len() == 1);
        if uncontested {
