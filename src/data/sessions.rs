...
pub async fn create_session(pool: &PgPool, user_id: i64, duration: std::time::Duration) -> Result<UserSession> {
    ...
    sqlx::query_as!(
        ...
        RETURNING id, user_id, created_at, expires_at, last_active_at
        "#,
        token,
        user_id,
        duration_secs_f64,
...
pub async fn delete_session(pool: &PgPool, token: &str) -> Result<()> {
    sqlx::query!("DELETE FROM user_sessions WHERE id = $1", token)
        .execute(pool)
        .await
...
pub async fn count_active(pool: &PgPool) -> Result<i64> {
    let count = sqlx::query_scalar!(r#"SELECT COUNT(*) AS "count!" FROM user_sessions WHERE expires_at > now()"#)
        .fetch_one(pool)
        .await?;
    Ok(count)
}
