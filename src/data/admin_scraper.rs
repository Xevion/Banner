...
pub async fn compute_stats(pool: &PgPool, interval_str: &str, term: Option<&str>) -> Result<ScraperStats> {
    ...
    Ok(ScraperStats {
        ...
        total_courses_changed: row.total_courses_changed,
        total_courses_fetched: row.total_courses_fetched,
        total_audits_generated: row.total_audits_generated,
        pending_jobs: queue_row.pending_jobs,
        locked_jobs: queue_row.locked_jobs,
    })
}
